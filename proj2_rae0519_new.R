...
dloop <- function(n, nreps = 10000){
  ...
  for (m in 1:nreps){
    ...
    for (i in 1:(2*n)){
      ...
      for(length_loop in 1:(2*n)){
        
        if (card == k[i]){
          success[length_loop] = 1
          break
        }
        card = u[card]
      }
    ...
    l <- which(success == 1)
    d_prob[l[length(l)]] <- d_prob[l[length(l)]] + 1
  }
  d_prob/sum(d_prob)
}
...
plot(dloop_probability, xlab="Loop Length",ylab="Probability")
...
for (i in 1:(2*n)){
  ...
  for(length_loop in 1:(2*n)){
    ...
    if (card == k[i]){
      success[length_loop] =  success[length_loop] + 1
      break
    }
    card = u[card]
  }
